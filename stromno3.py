...
def get_electricity_api_url():
    ...
    return f"https://www.hvakosterstrommen.no/api/v1/prices/{today}_NO3.json"
...
def get_reservoir_data():
    ...
    if response.status_code == 200:
        data = response.json()
        for item in data:
            if item["omrType"] == "EL" and item["omrnr"] == 3:
                return item
    print(f"Failed to retrieve reservoir data. Status code: {response.status_code}")
    return None

webflow_url = "https://api.webflow.com/v2/collections/66a7564313886c5b3e8443a0/items/66a75856cc7bc21132d42f23/live"
